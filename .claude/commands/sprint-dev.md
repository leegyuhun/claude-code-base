...
판정 기준의 강도는 정책을 따른다 (`python .claude/scripts/policy.py get auto_run`):
...
**후보 1건 이상 — 처리 방식은 정책을 따른다** (`python .claude/scripts/policy.py get pitfall_capture`):

| 값 | 동작 |
|---|---|
| `ask` (기본) | 아래 제안 후 [PAUSE] |
| `auto` | [PAUSE] 없이 전 후보를 append 하고 결과만 한 줄 보고 |
| `skip` | 기록하지 않고 후보 목록만 출력 후 6단계로 |
