...
```
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌──────────┐    ┌──────────┐
│   PRD   │ →  │  Plan   │ →  │  Plan   │ →  │ ROADMAP │ →  │  Init    │ →  │  GOAL    │
│  분석   │    │  생성   │    │  리뷰   │    │  세분화 │    │ (4.5)    │    │  작성    │
│ PHASE 1 │    │ PHASE 2 │    │ PHASE 3 │    │ PHASE 4 │    │ 초기화   │    │ PHASE 5  │
└─────────┘    └─────────┘    └─────────┘    └─────────┘    └──────────┘    └──────────┘
                                                                                   │
                                                                                   ▼
┌─────────┐    ┌─────────┐    ┌─────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐
...
| 에이전트 | PHASE | 역할 | 모델 |
|----------|-------|------|------|
| orchestrator | 1~4.5, 11 | PRD 분석, plan.md, ROADMAP.md, 초기화, Re-plan | opus |
...
|-----------|--------|--------|--------|
| 빌드 성공 (`dotnet build`) | ✅ 자동 | ✅ 자동 | ✅ 자동 |
| 코드 포맷 (`dotnet format`) | ✅ 자동 | — | — |
| 단위 테스트 (`dotnet test`) | ✅ 자동 | ✅ 타겟 | ✅ 전체 |
| API 엔드포인트 | ✅ 자동 | ✅ 타겟 | ✅ 전체 |
| 수동 UI/통합 테스트 | ⚠️ 수동 | ⚠️ 타겟 | ⚠️ 수동 |
| 헬스체크 | — | — | ✅ 자동 |
...
1. **자동 검증** (PHASE 7)
   - 빌드(`dotnet build`), 코드 포맷(`dotnet format`), 단위 테스트(`dotnet test`), API 응답
